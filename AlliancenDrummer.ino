...
uint32_t brazilColors[] = {red, green, yellow};
int numColors = sizeof(colors) / sizeof(colors[0]);
int numBrazilColors = sizeof(brazilColors) / sizeof(brazilColors[0]);
...
void loop()
{
  pixels.clear();
  if (colorWheel)
  { // Color wheel mode, when started with colorwheel switch turned on.
    if (SELECTORPIN)
    {
  ...
  { // Standard mode, when started with colorwheel switch turned off.
    if (digitalRead(MICPIN))
    {
